...
# dans les mêmes    # Légende dynamique selon le niveau
...
try:
    ...
    print("Les modules requis ne sont pas installés. Veuillez installer : pip install streamlit pandas")
    exit()
...
if type_apprentissage == "🔍 Apprentissage supervisé":
    ...
    # Initialisation de l'environnement amélioré
    if 'agent_position' not in st.session_state:
        ...
        st.session_state.visited_positions = set()
    
    # Affichage du niveau et progression
    col1, col2, col3, col4 = st.columns(4)
    with col1:
        st.metric("🎯 Niveau", st.session_state.game_level)
    with col2:
        st.metric("🏆 Meilleur Score", st.session_state.best_score)
    with col3:
        st.metric("📊 Épisodes", st.session_state.total_episodes)
    with col4:
        efficiency = round((st.session_state.treasure_found / max(st.session_state.steps_taken, 1)) * 100, 1) if st.session_state.steps_taken > 0 else 0
        st.metric("⚡ Efficacité", f"{efficiency}%")
    
    st.markdown("### 🗺️ Environnement : Monde de l'Agent Explorateur")
    ...
    # Génération d'environnement dynamique selon le niveau
    if st.session_state.game_level == 1:
        environment = ["🌱", "💰", "🤖", "💰", "🌱"]  # Niveau facile - plus de trésors
        level_name = "🟢 Novice"
    elif st.session_state.game_level == 2:
        environment = ["🕳️", "💰", "🤖", "🐍", "💎"]  # Niveau normal
        level_name = "🟡 Intermédiaire"
    elif st.session_state.game_level == 3:
        environment = ["🕳️", "🐍", "🤖", "🐍", "💎"]  # Niveau difficile - plus de dangers
        level_name = "🟠 Avancé"
    else:
        environment = ["🌋", "🐍", "🤖", "🐍", "💎"]  # Niveau expert - lave !
        level_name = "🔴 Expert"
    
    env_display = environment.copy()
    env_display[st.session_state.agent_position] = "🤖"
    
    st.markdown(f"### 🎮 **{level_name} - Votre environnement actuel :**")
    ...
    # Affichage amélioré de l'environnement
    cols = st.columns(5)
    position_names = ["Ouest", "Centre-Ouest", "Centre", "Centre-Est", "Est"]
    
    for i, (col, item, name) in enumerate(zip(cols, env_display, position_names)):
        with col:
            # Position actuelle de l'agent
            if i == st.session_state.agent_position:
                st.markdown(f"""
                <div style='text-align: center; background: linear-gradient(45deg, #90EE90, #32CD32); 
                           padding: 1.5rem; border-radius: 15px; border: 3px solid #228B22; margin: 0.5rem;'>
                    <div style='font-size: 3rem; margin-bottom: 0.5rem;'>{item}</div>
                    <div style='font-size: 0.8rem; color: #006400; font-weight: bold;'>{name}</div>
                    <div style='font-size: 0.7rem; color: #006400; margin-top: 0.2rem;'>🎯 VOUS ÊTES ICI</div>
                </div>
                """, unsafe_allow_html=True)
            # Positions visitées
            elif i in st.session_state.visited_positions:
                st.markdown(f"""
                <div style='text-align: center; background-color: #FFE4B5; 
                           padding: 1.5rem; border-radius: 15px; border: 2px dashed #DEB887; margin: 0.5rem;'>
                    <div style='font-size: 3rem; margin-bottom: 0.5rem; opacity: 0.7;'>{environment[i]}</div>
                    <div style='font-size: 0.8rem; color: #8B4513; font-weight: bold;'>{name}</div>
                    <div style='font-size: 0.7rem; color: #8B4513; margin-top: 0.2rem;'>✅ EXPLORÉ</div>
                </div>
                """, unsafe_allow_html=True)
            # Positions non explorées
            else:
                st.markdown(f"""
                <div style='text-align: center; background-color: #f0f2f6; 
                           padding: 1.5rem; border-radius: 15px; border: 1px solid #ddd; margin: 0.5rem;'>
                    <div style='font-size: 3rem; margin-bottom: 0.5rem;'>❓</div>
                    <div style='font-size: 0.8rem; color: #666; font-weight: bold;'>{name}</div>
                    <div style='font-size: 0.7rem; color: #666; margin-top: 0.2rem;'>🔍 MYSTÈRE</div>
                </div>
                """, unsafe_allow_html=True)
